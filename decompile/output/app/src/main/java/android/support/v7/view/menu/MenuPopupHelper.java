  ...
  implements MenuHelper
{
  ...
  private MenuPopup createPopup()
  {
    ...
    label189:
    label194:
    for (Object localObject = new CascadingMenuPopup(mContext, mAnchorView, mPopupStyleAttr, mPopupStyleRes, mOverflowOnly);; localObject = new StandardMenuPopup(mContext, mMenu, mAnchorView, mPopupStyleAttr, mPopupStyleRes, mOverflowOnly))
    {
  ...
  private void showPopup(int paramInt1, int paramInt2, boolean paramBoolean1, boolean paramBoolean2)
  {
    ...
    if (paramBoolean1)
    {
      if ((0x7 & GravityCompat.getAbsoluteGravity(mDropDownGravity, ViewCompat.getLayoutDirection(mAnchorView))) == 5) {
        paramInt1 -= mAnchorView.getWidth();
      }
      localMenuPopup.setHorizontalOffset(paramInt1);
      localMenuPopup.setVerticalOffset(paramInt2);
      int i = (int)(48.0F * mContext.getResources().getDisplayMetrics().density / 2.0F);
